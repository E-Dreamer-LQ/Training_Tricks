...
from model.fpn.bbox_transform import bbox_transform_inv, bbox_overlaps
...
def repulsion(rois, box_deltas, gt_rois, rois_inside_ws, rois_outside_ws):
    ...
    batch_size = rois.shape[1]/128
    rois = rois.view(int(batch_size),128,5)
    deltas = Variable(box_deltas.view(rois.shape[0],128,4))
    rois_inside_ws = Variable(rois_inside_ws.view(rois.shape[0],128,4))
    rois_outside_ws = Variable(rois_outside_ws.view(rois.shape[0],128,4))
    ...
    pred_boxes = pred_boxes.view(int(batch_size),128,4)
