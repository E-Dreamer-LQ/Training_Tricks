...
# 来自： https://github.com/Tianxiaomo/pytorch-YOLOv4/blob/master/dataset.py
...
def fill_truth_detection(bboxes, num_boxes, classes, flip, dx, dy, sx, sy, net_w, net_h): # 读取图片对应的标注信息
    if bboxes.shape[0] == 0:
        return bboxes, 10000
...
class Yolo_dataset(Dataset):
    def __init__(self, lable_path, cfg):
        ...
        import glob
        xml_list = glob.glob(os.path.join(lable_path,"*xml"))
...
if __name__ == "__main__":
    ...
    random.seed(2020)
    np.random.seed(2020)
    Cfg.dataset_dir = '/home/lab30202/sdb/liuqiang/2020-6-1-mosaic_data_augumention/test_data/Image'
    dataset = Yolo_dataset(Cfg.train_label, Cfg) #  train_label 读取出来为 图像名称 和 所有目标的(x1.x2,y1,y2) , ori 为读取 train.txt文件
    for i in range(10):
        out_img, out_bboxes = dataset.__getitem__(i)
        a = draw_box(out_img.copy(), out_bboxes.astype(np.int32))
        #plt.imshow(a.astype(np.int32))
        #plt.show()
        cv2.imwrite('/home/lab30202/sdb/liuqiang/2020-6-1-mosaic_data_augumention/results/test.jpg',a)
        print("hello")
